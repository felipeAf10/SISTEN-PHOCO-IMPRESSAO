...
const AutomotiveModal: React.FC<AutomotiveModalProps> = ({ isOpen, onClose, onConfirm, activeProduct }) => {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
            {/* Added bg-surface-active explicitly */}
            <div className="glass-card w-full max-w-6xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] bg-surface-active/95 border border-white/10 text-primary">
                {/* Header */}
                <div className="p-4 lg:p-8 bg-surface-active text-white flex justify-between items-center shrink-0 border-b border-white/5">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-secondary text-xs font-bold uppercase mt-1">Cálculo por Peças e Complexidade</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-col lg:flex-row flex-1 overflow-hidden bg-surface/50">
                    ...
                    <div className="flex-1 p-4 lg:p-8 overflow-y-auto custom-scrollbar space-y-8">
                        ...
                        {/* Search Car */}
                        <div className="bg-surface-active border border-white/10 p-6 rounded-[2rem] space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                                <input placeholder="Marca (Ex: Fiat)" value={autoForm.make} onChange={e => setAutoForm({ ...autoForm, make: e.target.value })} className="px-5 py-4 bg-input border-none rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-fuchsia-500 text-white placeholder-secondary shadow-inner" />
                                <input placeholder="Modelo (Ex: Fiorino)" value={autoForm.model} onChange={e => setAutoForm({ ...autoForm, model: e.target.value })} className="px-5 py-4 bg-input border-none rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-fuchsia-500 text-white placeholder-secondary shadow-inner" />
                                <div className="flex gap-2">
                                    <input placeholder="Ano" value={autoForm.year} onChange={e => setAutoForm({ ...autoForm, year: e.target.value })} className="flex-1 px-5 py-4 bg-input border-none rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-fuchsia-500 text-white placeholder-secondary shadow-inner" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-secondary uppercase tracking-widest">Nível de Material</label>
                                <select value={materialLevel} onChange={e => setMaterialLevel(e.target.value as any)} className="w-full px-5 py-4 bg-input border border-white/10 rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-fuchsia-500 text-white">
                                    {Object.keys(MATERIAL_LEVELS).map(k => <option key={k} value={k}>{k}</option>)}
                                </select>
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-secondary uppercase tracking-widest">Complexidade / Curvas</label>
                                <select value={complexity} onChange={e => setComplexity(e.target.value as any)} className="w-full px-5 py-4 bg-input border border-white/10 rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-fuchsia-500 text-white">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                    {Object.entries(autoBreakdown).map(([part, dim]) => (
                                        <button
                                            ...
                                            className={`p-3 rounded-xl border flex flex-col items-start gap-1 transition-all ${selectedAutoParts.has(part) ? 'bg-fuchsia-600/20 border-fuchsia-500 text-white' : 'bg-surface-hover border-white/5 text-secondary hover:border-white/20 hover:bg-surface-active'}`}
                                        >
