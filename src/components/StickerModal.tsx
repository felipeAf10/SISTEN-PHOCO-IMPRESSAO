...
const StickerModal: React.FC<StickerModalProps> = ({ isOpen, onClose, onConfirm, activeProduct }) => {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
            {/* Added bg-surface explicitly here to ensure dark theme even if glass-card fails */}
            <div className="glass-card w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-white/10 bg-surface text-primary">

                {/* Header */}
                <div className="p-4 lg:p-8 bg-surface text-white flex justify-between items-center shrink-0 border-b border-white/5">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-secondary text-xs font-bold uppercase mt-1">Otimização e Corte</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="bg-white/5 p-2 rounded-full text-secondary hover:text-white hover:bg-white/10 transition-colors"><X size={24} /></button>
                </div>

                <div className="flex flex-col lg:flex-row flex-1 overflow-hidden bg-surface/50">
                    {/* Controls */}
                    <div className="flex-1 p-4 lg:p-8 overflow-y-auto custom-scrollbar space-y-8">
                        ...
                        <div className="flex bg-surface-hover/50 p-1.5 rounded-2xl border border-white/5">
                            <button
                                ...
                                className={`flex-1 py-3 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'quantity' ? 'bg-indigo-600 shadow-lg shadow-indigo-500/20 text-white' : 'text-secondary hover:text-primary hover:bg-surface'}`}
                            >
                                Tenho a Quantidade
                            </button>
                            <button
                                onClick={() => setMode('area')}
                                className={`flex-1 py-3 px-6 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${mode === 'area' ? 'bg-indigo-600 shadow-lg shadow-indigo-500/20 text-white' : 'text-secondary hover:text-primary hover:bg-surface'}`}
                            >
                                Tenho a Área (M²)
                            </button>
                        </div>
                        ...
                        <div className="space-y-6">
                            <h5 className="text-[10px] font-black text-secondary uppercase tracking-widest flex items-center gap-2">
                                <Grid size={14} /> Configuração da Unidade - {activeProduct?.name}
                            </h5>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[9px] font-black text-secondary uppercase">Largura (cm)</label>
                                        <input type="number" value={dims.width} onChange={e => setDims({ ...dims, width: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-input border border-white/10 rounded-xl font-black outline-none focus:ring-2 focus:ring-indigo-500 text-white placeholder-secondary/50" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[9px] font-black text-secondary uppercase">Altura (cm)</label>
                                        <input type="number" value={dims.height} onChange={e => setDims({ ...dims, height: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-input border border-white/10 rounded-xl font-black outline-none focus:ring-2 focus:ring-indigo-500 text-white placeholder-secondary/50" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[9px] font-black text-secondary uppercase">Sangria (mm)</label>
                                        <input type="number" value={dims.gapMm} onChange={e => setDims({ ...dims, gapMm: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-input border border-white/10 rounded-xl font-black outline-none focus:ring-2 focus:ring-indigo-500 text-white placeholder-secondary/50" />
                                    </div>
                                </div>
                                ...
                                <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-10 pr-4 py-3 bg-input border border-amber-500/30 rounded-xl font-black text-white outline-none focus:ring-2 focus:ring-amber-500 placeholder-secondary/50"
                                        />
                                    </div>
                                </div>
                                ...
                                {mode === 'quantity' ? (
                                    <div className="space-y-2 animate-in fade-in slide-in-from-left-4">
                                        <label className="text-[10px] font-black text-secondary uppercase tracking-widest">Quantidade Desejada</label>
                                        <input type="number" value={targetQty} onChange={e => setTargetQty(parseInt(e.target.value) || 0)} className="w-full px-5 py-4 bg-input border border-white/10 rounded-2xl font-black text-lg outline-none focus:ring-2 focus:ring-indigo-500 text-indigo-400 placeholder-secondary/50" />
                                    </div>
                                ) : (
                                    <div className="space-y-2 animate-in fade-in slide-in-from-right-4">
                                        <label className="text-[10px] font-black text-secondary uppercase tracking-widest">Área de Preenchimento (Metros)</label>
                                        <div className="flex gap-4">
                                            <input type="number" step="0.01" value={targetArea.w} onChange={e => setTargetArea({ ...targetArea, w: parseFloat(e.target.value) || 0 })} className="flex-1 px-5 py-4 bg-input border border-white/10 rounded-2xl font-black text-lg outline-none focus:ring-2 focus:ring-indigo-500 text-indigo-400 placeholder-secondary/50" placeholder="Largura (m)" />
                                            <span className="self-center font-black text-secondary">x</span>
                                            <input type="number" step="0.01" value={targetArea.h} onChange={e => setTargetArea({ ...targetArea, h: parseFloat(e.target.value) || 0 })} className="flex-1 px-5 py-4 bg-input border border-white/10 rounded-2xl font-black text-lg outline-none focus:ring-2 focus:ring-indigo-500 text-indigo-400 placeholder-secondary/50" placeholder="Altura (m)" />
                                        </div>
                                    </div>
                                )}
                            </div>

                            <div className="p-4 bg-surface/30 border border-white/5 rounded-2xl flex gap-3 text-secondary">
                        ...
                        <div className="w-full lg:w-[400px] bg-gradient-to-br from-indigo-600 to-indigo-900 text-white p-8 flex flex-col justify-between relative overflow-hidden shadow-inner border-t lg:border-t-0 lg:border-l border-white/10">
                            ...
                            <button
                                ...
                                className="w-full py-5 bg-white text-indigo-700 hover:brightness-95 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed mt-8"
                            >
