...
const ProductList: React.FC<ProductListProps> = ({ products, setProducts, costPerHour, finConfig, initialSearch = '' }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const salePrice = calculateSalePrice(finalCost, formData.productionTimeMinutes, formData.wastePercent);

    if (isNaN(salePrice) || isNaN(Number(finalCost))) {
      alert("Erro no valor. Verifique se digitou um número válido (ex: 10,50 ou 10.50).");
      return;
    }
    ...
    try {
      ...
      if (editingProduct) {
        const updatedProduct: Product = { ...editingProduct, ...cleanData, salePrice };
        await api.products.update(updatedProduct);
        setProducts(prev => prev.map(p => p.id === editingProduct.id ? updatedProduct : p));
      } else {
        const newProduct: Product = { id: crypto.randomUUID(), ...cleanData, salePrice };
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => {
                      // Allow user to type, we parse on Save
