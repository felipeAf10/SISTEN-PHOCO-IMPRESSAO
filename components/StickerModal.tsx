...
const StickerModal: React.FC<StickerModalProps> = ({ isOpen, onClose, onConfirm, activeProduct }) => {
    ...
    React.useEffect(() => {
        if (activeProduct) {
            ...
            console.log("StickerModal Price Init:", validPrice);
    ...
    const calculation = useMemo(() => {
        ...
        if (mode === 'quantity') {
            ...
            // Fit in Area Mode
            // We have a board of W x H. 
            // How many fit in this board?
            // Note: This assumes we are printing ON a sheet of this size, OR cutting from a roll to fit this size.
            // Simplest: Calculate how many fit in W x H rectangle.
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
            <div className="glass-card w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-white/10 bg-slate-900/90">
                ...
                <div className="p-4 lg:p-8 bg-[#0F172A] text-white flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h4 className="text-2xl font-black uppercase tracking-tight">Estúdio de Adesivos</h4>
                            <p className="text-slate-400 text-xs font-bold uppercase mt-1">Otimização e Corte</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="text-slate-500 hover:text-white transition-colors"><X size={32} /></button>
                </div>
                ...
                <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="space-y-1">
                                    ...
                                    <input type="number" value={dims.width} onChange={e => setDims({ ...dims, width: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl font-black outline-none focus:ring-2 focus:ring-indigo-500 text-slate-200" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[9px] font-black text-slate-500 uppercase">Altura (cm)</label>
                                    <input type="number" value={dims.height} onChange={e => setDims({ ...dims, height: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl font-black outline-none focus:ring-2 focus:ring-indigo-500 text-slate-200" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[9px] font-black text-slate-500 uppercase">Sangria (mm)</label>
                                    <input type="number" value={dims.gapMm} onChange={e => setDims({ ...dims, gapMm: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl font-black outline-none focus:ring-2 focus:ring-indigo-500 text-slate-200" />
                                </div>
                            </div>
                            ...
                            <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl space-y-2">
                                ...
                                <div className="relative">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-400 font-bold text-xs">R$</span>
                                    <input
                                        type="number"
                                        step="0.01"
                                        value={pricePerM2}
                                        onChange={e => setPricePerM2(parseFloat(e.target.value) || 0)}
                                        className="w-full pl-10 pr-4 py-3 bg-slate-800 border border-amber-500/30 rounded-xl font-black text-slate-200 outline-none focus:ring-2 focus:ring-amber-400"
                                    />
                                </div>
                            </div>
                            ...
                            {mode === 'quantity' ? (
                                <div className="space-y-2 animate-in fade-in slide-in-from-left-4">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Quantidade Desejada</label>
                                    <input type="number" value={targetQty} onChange={e => setTargetQty(parseInt(e.target.value) || 0)} className="w-full px-5 py-4 bg-slate-800 border border-slate-700 rounded-2xl font-black text-lg outline-none focus:ring-2 focus:ring-indigo-500 text-indigo-400" />
                                </div>
                            ) : (
                                <div className="space-y-2 animate-in fade-in slide-in-from-right-4">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Área de Preenchimento (Metros)</label>
                                    <div className="flex gap-4">
                                        <input type="number" step="0.01" value={targetArea.w} onChange={e => setTargetArea({ ...targetArea, w: parseFloat(e.target.value) || 0 })} className="flex-1 px-5 py-4 bg-slate-800 border border-slate-700 rounded-2xl font-black text-lg outline-none focus:ring-2 focus:ring-indigo-500 text-indigo-400" placeholder="Largura (m)" />
                                        <span className="self-center font-black text-slate-500">x</span>
                                        <input type="number" step="0.01" value={targetArea.h} onChange={e => setTargetArea({ ...targetArea, h: parseFloat(e.target.value) || 0 })} className="flex-1 px-5 py-4 bg-slate-800 border border-slate-700 rounded-2xl font-black text-lg outline-none focus:ring-2 focus:ring-indigo-500 text-indigo-400" placeholder="Altura (m)" />
                                    </div>
                                </div>
                            )}
                        </div>

                        <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-2xl flex gap-3 text-slate-400">
                            <Info size={20} className="shrink-0 mt-1 text-indigo-400" />
                    ...
                    {/* Solution Preview */}
                    <div className="w-full lg:w-[400px] bg-gradient-to-br from-indigo-600 to-indigo-800 text-white p-8 flex flex-col justify-between relative overflow-hidden shadow-inner">
                        {/* Background Pattern */}
                        ...
                        <div className="space-y-8 relative z-10">
                            ...
                            <div className="bg-white/10 rounded-2xl p-6 space-y-3 backdrop-blur-sm">
                            ...
                            <div className="text-right">
                                <p className="text-3xl font-black">R$ {(pricePerM2 * calculation.finalArea).toFixed(2)}</p>
                                {pricePerM2 === 0 && (
                                    <p className="text-[9px] text-amber-300 font-bold mt-1 max-w-[200px] leading-tight">⚠ Defina um preço por m² para calcular.</p>
                                )}
                        ...
                        <button
                            ...
                            className="w-full py-5 bg-white text-indigo-600 hover:bg-indigo-50 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
