...
const IndicatorPanel: React.FC<IndicatorPanelProps> = ({ items, total, designFee, installFee, config, onClose, onSave }) => {
  ...
  const matPercent = (materialCost / prodCostTotal) * 100;
  const labPercent = (laborCost / prodCostTotal) * 100;
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
        ...
        {/* Tabs - Visual Only for Design */}
        <div className="px-8 pt-4 border-b border-slate-100 flex gap-10">
           <button className="pb-4 text-xs font-black uppercase tracking-widest text-slate-400">Custos</button>
           <button className="pb-4 text-xs font-black uppercase tracking-widest text-slate-400">Medidas</button>
           <button className="pb-4 border-b-4 border-indigo-600 text-xs font-black uppercase tracking-widest text-indigo-600">Indicadores</button>
        </div>
        ...
        <div className="p-8 border-t border-slate-100 bg-slate-50/80 flex flex-wrap gap-4 justify-end shrink-0">
           ...
           <button className="px-8 py-4 bg-white border border-indigo-200 text-indigo-600 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-indigo-50 flex items-center gap-2 transition-all active:scale-95">
              <ClipboardCheck size={16}/> Checklist
           </button>
           <button className="px-8 py-4 bg-gradient-to-br from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-white rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 shadow-lg shadow-amber-500/20 transition-all active:scale-95">
              <RefreshCcw size={16}/> Recalcular
           </button>
           <button onClick={onSave} className="px-12 py-4 bg-gradient-to-br from-[#00899B] to-[#007685] hover:from-[#00a2b5] hover:to-[#00899B] text-white rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 shadow-xl shadow-cyan-500/25 transition-all active:scale-[0.98]">
